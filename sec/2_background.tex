...
The transformer architecture, introduced in \cite{vaswani2017attention}, has become ubiquitous accross various domains in machine learning. This is a sequence-to-sequence architecture that uses only attention:
\[
\text{Attention}(Q, K, V) = \mathrm{softmax}\!\left(\frac{QK^{\top}}{\sqrt{d_k}}\right)V
\]
Given that the attention mechanism is perumtation invaraint, positonal embeddings are added to the token embeddings to encode relative and absolute position in the sequence. Transformers have revolutionized language models by allowing all tokens in a sequence to attend to each other. \cite{dosovitskiy2021vit} applied this architecture to images by treating patches as words, showing that this revolutionary design was not limited to only natural language processing tasks. Compared to standard convolutional inductive biases, ViTs do not assume locality or translation invariance. Instead, they learns all structure from data, including which regions interact.
