...
Much of the excitement around modern generative models comes from treating generation as a transport problem instead of a one-shot pixel prediction task. The idea is to map a simple distribution (e.g. Gaussian noise) into the data distribution (e.g. photos of cats) by learning how samples should move over time. Diffusion and score-based models do this by predicting a time-dependent direction at every point so that integrating a neural ODE gradually denoises the sample. The catch is that these trajectories are usually quite curved around the data manifold, so numerical solvers end up taking hundreds of tiny steps to stay accurate.

If the path were nearly straight, fewer solver steps would be required. Probability-flow ODEs already provide a deterministic way to trace diffusion trajectories, but they still inherit the curved flows learned by the teacher model. Rectified Flow \cite{liu2023rectifiedflow} explicitly learns a ``rectified'' velocity field from a diffusion teacher so the transport path is as straight as possible, which in practice reduces sampling to tens of steps without sacrificing image quality.
